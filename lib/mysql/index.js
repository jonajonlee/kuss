...
const escapeValue = R.curry((mysql, val) => mysql.escape(val))
...
, R.join(', ')
, R.map(R.unless(R.equals('*'), escapeId(mysql)))
)(projection)
...
// _keysToSqlColumns :: MySqlConnection -> [ String ] -> String
// Example: [ 'a', 'b', 'c' ] -> "(`a`, `b`, `c`)"
const _keysToSqlColumns = R.curry((mysql, keys) => R.compose(
  (x) => `(${x})`
, R.join(',')
...
// _keysToSqlDuplicateUpdate :: MySqlConnection -> [ String ] -> String
// Example: [ 'a', 'b', 'c' ] -> "a=VALUES(a),b=VALUES(b),c=VALUES(c)"
const _keysToSqlDuplicateUpdate = R.curry((mysql, keys) => R.compose(
  R.join(',')
, R.map(R.compose(
    (x) => `${x}=VALUES(${x})`
  , escapeId(mysql)
...
// _objectToInsertValues :: MySqlConnection -> [ String ] -> Object -> String
// Example: [ 'a', 'b', 'c' ], { a: 1, b: 'b' } -> "(1,\"b\",DEFAULT)"
const _objectToInsertValues = R.curry((mysql, keys, obj) => R.compose(
  (x) => `(${x})`
, R.join(',')
, R.map(R.ifElse(
    R.equals(undefined)
...
// objectsToBulkInsertValues ::
//   MySqlConnection -> [ String ] -> [ Object ] -> String
// Example:
//   [ 'a', 'b', 'c' ],
//   [ { a: 1, b: 2 }, { a: 1, c: 'c' } ]
//   -> "(1,2,DEFAULT),(1,DEFAULT,\"c\")"
const _objectsToBulkInsertValues = R.curry((mysql, keys, list) => R.compose(
  R.join(',')
, R.map(_objectToInsertValues(mysql, keys))
)(list))
...
const bulk_insert = R.curry((mysql, table, params) => {
  ...
  const columns     = _keysToSqlColumns(mysql, unique_keys)
  const values      = _objectsToBulkInsertValues(mysql, unique_keys, params)
...
const bulk_update = R.curry((mysql, table, params) => {
  ...
  const columns     = _keysToSqlColumns(mysql, unique_keys)
  const values      = _objectsToBulkInsertValues(mysql, unique_keys, params)
  const updates     = _keysToSqlDuplicateUpdate(mysql, unique_keys)
...
const bulk_upsert = R.curry((mysql, table, keys, params_list) => {
  ...
  return Bluebird.map(params_list, (param) =>
    ...
    , (res) => {
        ...
        update_bucket.push(R.merge(existing, param))
      }
    ))
  )
  .then(() => Bluebird.all([
